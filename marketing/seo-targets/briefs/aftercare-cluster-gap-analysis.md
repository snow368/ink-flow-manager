...
> **动笔 `tattoo-aftercare-guide` 前逐项核对。** 本清单合并四份依据：① 写作规则 `site-content-seo-ruleset.md`；② E-E-A-T 规则 `eeat-authority-source-plan.md` + 用户级 E-E-A-T 强制信号；③ SERP 必含段（§4.2①）；④ Gap 修正（§7.4 词数 + §7.3 结构/内链/Meta）。写的时候按这份来，不要只盯 §7.6 单条。
...
### 8.3 结构调整（KB §7.3 三原则）
- [ ] **BLUF**：首段 40–60 字直给答案（"tattoo aftercare = 清洁 + 保湿 + 防晒 + 别抠"）
- [ ] 每段自包含；关键对比用**表格**（产品对比表 / by-type 对照表 / 并发症对照表）
...
5. 再 cascade 6 spoke（顺序 products → issues → timeline → color → second-skin → activities）
