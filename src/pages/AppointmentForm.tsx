...
export default function AppointmentForm() {
  ...
  const loadClients = () => {
    ...
    db.clients.orderBy('createdAt').reverse().toArray()
      .then(list => {
        setClients(list);
        if (list.length === 0) setError('');  // 有客户时清除错误
      })
  ...
  const handleQuickCreateClient = async () => {
    ...
    try {
      ...
      setError('Failed to create client: ' + (e?.message || 'unknown'));
    } finally {
      ...
      {showQuickCreate ? (
        <div style={{ background: '#1e293b', padding: 16, borderRadius: 12, marginBottom: 16 }}>
          ...
          <button
            ...
            disabled={creatingClient || !quickName.trim()}
            style={{
              width: '100%', padding: 12, borderRadius: 10, border: 'none',
              background: creatingClient || !quickName.trim() ? '#4b5563' : '#e11d48',
              color: 'white', fontSize: 14, fontWeight: 600,
            }}
          >
            {creatingClient ? 'Adding...' : 'Add Client & Continue'}
          </button>
          <p
            onClick={() => navigate('/client/new')}
            style={{ textAlign: 'center', marginTop: 12, color: '#94a3b8', fontSize: 13, cursor: 'pointer', textDecoration: 'underline' }}
          >
            or create a full client profile
          </p>
        </div>
      ) : (
        <>
          {loadingClients ? (
            ...
            <select
              ...
              onChange={e => { setSelectedClient(e.target.value); setError(''); }}
              style={selectStyle}
            >
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
            {durationPresets.map(preset => (
              <button
                key={preset.value}
                onClick={() => { setDuration(preset.value); setCustomDuration(''); }}
                style={{
                  flex: 1, padding: '10px 0', borderRadius: 10, border: 'none',
                  background: (duration === preset.value && !customDuration) ? '#e11d48' : '#334155',
                  color: 'white', fontSize: 14, fontWeight: 600,
                }}
              >
                {preset.label}
          ...
          <input
            ...
            onChange={e => { setCustomDuration(e.target.value); if (e.target.value) setDuration(0); }}
            style={inputStyle}
          />
          ...
          <button
            ...
            style={{
              ...
              background: saving || !selectedClient ? '#4b5563' : '#e11d48',
              color: 'white', fontSize: 16, fontWeight: 600,
              cursor: saving || !selectedClient ? 'not-allowed' : 'pointer',
            }}
          >
